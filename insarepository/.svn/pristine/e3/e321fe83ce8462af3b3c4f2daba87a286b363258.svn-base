...
public class EmpReportController {
	...
	public ModelAndView requestEmployment(HttpServletRequest request, HttpServletResponse response) {  //재직증명서 신청
		...
		try {
			...
			//JasperFillManager= 보고서디자인에데이터를 채우는 클래스 
			...
			JasperExportManager.exportReportToPdfFile(jasperPrint,(request.getServletContext().getRealPath("/report/"+empCode+"_"+usage+".pdf"))); 
			JasperExportManager.exportReportToPdfStream(jasperPrint, outputStream);
			//JasperExportManager = 생성된보고서를 pdf,html,xml 형식으로 내보내는 class 
			// 첫번째 매개변수로 생성된 보고서를 pdf 형식으로 두번째 매개변수로 지정된 출력스트림에 사용함
			outputStream.flush();
	...
	@RequestMapping("base/empSalReport.do")
	public ModelAndView requestMonthSalary(HttpServletRequest request, HttpServletResponse response) { //월급여신청
		...
		try {
			ReportSalaryTO to = baseServiceFacade.viewSalaryReport(parameters);
			Map<String, Object> map = new HashMap<String, Object>();
